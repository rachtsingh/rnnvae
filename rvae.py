...
class RVAE(nn.Module):
    ...
    We're using a single layer RNN on both the encoder and decoder side, and removing the option to tie
    the encoder and decoder embeddings together
    """
    ...
    def __init__(self, ntoken, ninp, nhid, z_dim, nlayers, dropout=0.5, dropouth=0.5,
        ...
        self.dec = nn.ModuleList([self.dropout, self.dec_embedding, self.decoder, self.out_embedding])
    ...
    def elbo(self, logits, targets, criterion, mean, logvar, args, anneal, prior_mean=None, prior_logvar=None):
        """
        prior_mean/prior_logvar aren't currently used; they are to allow fitted priors
        """
        seq_len, batch_size, ntokens = logits.size()
        NLL = seq_len * batch_size * criterion(logits.view(-1, ntokens), targets.view(-1))
        # KL = 0.5 * torch.sum((logvar.exp() + (mean - prior_mean).pow(2))/prior_logvar.exp() - 1. +
        #                      prior_logvar - logvar)
